...
class NoiseNode:
    ...
    def lora_cb(self, lora):                                            # lora callback-handler for data-transmission
    ...
    def confirmed_data_transmission_mode(self):                               # ask for confirmation of transmitted packets via the lora_socket
        ...
        self.s.setblocking(False)                                       # make the socket non blocking default
    ...
    def send_sound(self):                                               # sendig dummy data over lora
        self._log('Recolting sound sensor data...')
        data = [1,2,3,4,1,2,3,4]
        pkt = struct.pack('>%sb' % (len(data)), *data)
        self.send_packet(pkt, rx_ON=False)

    def simulate_sensor_data_transmission(self):                        # sendig repititive dummy data over lora
        i = 11
        while True:
